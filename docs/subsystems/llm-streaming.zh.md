...
- **仅靠空闲 watchdog 无法为流设定上限，因为传输活动不等于进展。** keep-alive 注释也是字节，而字节会重置 watchdog，因此一个返回 HTTP 200 后只发送 `: keep-alive` 的提供方会把调用一直挂到它自己的截断时刻（2026-09-14 在 `deepseek-flash` 上实测：约十五分钟、零负载，随后在没有 `[DONE]` 的情况下 EOF，协议将其报告为不可重试的 `STREAM_CLOSED`）。因此 DeepSeek 适配器新增 `streamFirstPayloadTimeoutMs`：对“发送请求到第一个 data 负载”这一窗口设定默认 25 秒的上限，注释不会延长它；在第一个负载之前注释不再重置空闲 watchdog，在此之后注释与既往完全一致，而到期被映射为 `TIMEOUT`，默认重试策略已能恢复。mock server 的 `keepalive_stall` 行为可复现该形态。
