...
- **提供方停顿在传输层受到时限约束。** 两个已交付的远程适配器都暴露正数且有限的 `streamIdleTimeoutMs`，默认五分钟。watchdog 只在 iterator `next()` 尚未完成时启动，整个请求使用同一个稳定 signal，把自身到期映射为 `TIMEOUT`，并把更早发生的调用方中止保留为 `ABORTED`。由于心跳会重新启动该 watchdog，DeepSeek 适配器还限制等待首个内容事件的时间（`streamFirstPayloadTimeoutMs`，默认 25 秒）：只发送 keep-alive 注释、`ping` 或 `message_start` 的流会以可重试的 `TIMEOUT` 失败，而不是一直挂起到提供方自身的截止时间。
