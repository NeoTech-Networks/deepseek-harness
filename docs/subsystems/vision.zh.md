...
除非 `subagent-model-selection` 用户设置（「允许 Agent 为 Subagent 选择模型」开关）已启用且至少命名一个候选路由，否则该服务处于不活跃状态。视觉路由是第一个其模型声明 `image` 输入的被允许路由；若都不符合，则回退到配置的 `visionRoute`（`deepseek-official` / `deepseek-flash`）。开关关闭时，纯文本选择保持现有的「Model does not support image input」拒绝。
