...
- **Provider stalls are bounded at the transport.** Both shipping remote adapters expose positive finite `streamIdleTimeoutMs` with a five-minute default. The watchdog arms only while iterator `next()` is outstanding, uses one stable signal for the whole request, maps its own expiry to `TIMEOUT`, and keeps an earlier caller abort as `ABORTED`. Because heartbeats rearm that watchdog, the DeepSeek adapter also bounds the wait for the first content event (`streamFirstPayloadTimeoutMs`, default 25 seconds): a stream that sends only keep-alive comments, `ping` or `message_start` fails as a retryable `TIMEOUT` instead of hanging until the provider's own cut-off.
