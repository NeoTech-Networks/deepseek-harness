...
- **An idle watchdog alone cannot bound a stream, because transport activity is not progress.** A keep-alive comment is bytes, and bytes rearm the watchdog, so a provider that returns HTTP 200 and then emits only `: keep-alive` holds the call open until its own cut-off (observed 2026-09-14 on `deepseek-flash`: about fifteen minutes, zero payloads, then EOF without `[DONE]`, which the protocol reports as the non-retryable `STREAM_CLOSED`). The DeepSeek adapter therefore adds `streamFirstPayloadTimeoutMs`, a two-minute default bound on send-to-first-data-payload that comments do not extend; before the first payload a comment no longer rearms the idle watchdog, after it comments count exactly as before, and expiry is `TIMEOUT`, which the default retry policy already recovers. The mock server's `keepalive_stall` behavior reproduces the shape.
