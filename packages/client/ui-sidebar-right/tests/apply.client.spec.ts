...
describe('ui-sidebar-right apply', () => {
  ...
  it('hands the panel seat the frame report, the service binding, the opens, the observable registry, and the Tab domain', async () => {
    ...
    // The reporting session keys the frame's saved width (fork).
    const reporting = 'report-session' as SessionId
    injected.syncPresentation({ shown: true, track: true, fullscreen: false }, reporting)
    expect(layout.openRightbar).toHaveBeenLastCalledWith(true, false, reporting)
    injected.syncPresentation({ shown: true, track: true, fullscreen: true }, reporting)
    expect(layout.openRightbar).toHaveBeenLastCalledWith(true, true, reporting)
    injected.syncPresentation({ shown: true, track: false, fullscreen: true }, reporting)
    expect(layout.openRightbar).toHaveBeenLastCalledWith(false, true, reporting)
    injected.syncPresentation({ shown: false, track: false, fullscreen: false }, reporting)
