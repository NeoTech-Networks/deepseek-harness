---
description: "The right Sidebar's pinned-directory explorer: operator-owned roots anywhere on the Host, listed lazily, with its own file preview and an opt-in once-per-Session auto-open."
...
- **The explorer type** — kind `explorer`, id `@deepseek-ai/dsh-client-ui-sidebar-explorer`, band `builtin`, no patterns, one guide entry (id `pinned`) at order 5.
...
- **The auto-open** — one subscription to `ctx.sidebarRight.mounted`, inert unless `autoOpen` is on.
...
The Sidebar keeps no layout across reloads and draws nothing before a Session's surface has been mounted, so "the explorer is simply there" cannot be a property of the layout. This package takes it as an action instead: on each change of `ctx.sidebarRight.mounted` (the Session whose right-Sidebar seat is drawn) it calls `openTabIn(sessionId, 'explorer')`, once per Session, with one late retry that covers the surface's first paint. Opening content expands the column, which is why each Session is visited at most once: collapse it and it stays collapsed. It is off by default, matching the upstream file tree, which never opens itself; setting the live `autoOpen` field of the `pinned-files` entry to `true` turns it on, and the value is read once at load.
...
- **No reordering or renaming of roots.** A root's label is its basename, and the order is the order they were added; changing either means editing the `pinned-files` entry.
...
- **`autoOpen` is read once.** Editing it on the `pinned-files` entry takes effect at the next load, unlike the roots themselves, which are re-read on every refresh.
