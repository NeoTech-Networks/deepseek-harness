 ...
 */
export const inject = ['uiWorkspace', 'slots', 'locale', 'sidebarRightTabs']
  ...
  ctx.effect(() => ctx.slots.inject('sidebar.right.pane.tab', () => ctx.slots.register(
    {
      ...
      key: SESSIONS_ID,
      locale: NS,
      // 0.1.7 moved opening to the workspace navigation service.
      inject: (): SessionsPanelInjected => ({
        open: (sessionId: SessionId) => { (ctx.get('uiWorkspace') as UiWorkspace).openSession(sessionId) },
      }),
    },
    SessionsPanel,
