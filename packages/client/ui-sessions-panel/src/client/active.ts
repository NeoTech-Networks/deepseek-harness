...
function phaseOf(
  ...
  status: ReturnType<SessionStatusSnapshot['get']>,
): SessionPanelPhase {
  ...
  if (status?.running ?? summary.running) return 'running'
...
/** The main-view Session, the one the conversation shows (0.1.7 retention rule). */
 ...
 * @param statuses - unified UI status (running, pending interaction, completion) keyed by session.
...
export function deriveSessions(
  ...
  for (const id of list.ids) {
    ...
    if (summary === undefined || !visible(summary, current, archived)) continue
    const phase = phaseOf(summary, runningCounts.get(id) ?? 0, statuses.get(id))
