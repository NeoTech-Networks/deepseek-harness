 ...
 * slot, locale, and navigation faces are recorders, because what matters here is
...
async function boot() {
  ...
  await fiber.await()
  return { tabs, registered, dictionaries, uiWorkspace, fiber }
}
...
describe('ui-sessions-panel apply', () => {
  ...
  it('injects an open action that opens the session through the workspace navigation', async () => {
    const { registered, uiWorkspace } = await boot()
    const face = registered[0]?.inject?.() as { open: (id: string) => void }
    expect(typeof face.open).toBe('function')
    face.open('s-test')
    expect(uiWorkspace.openSession).toHaveBeenCalledWith('s-test')
  })
