...
- **控制台隔离不可用。** 以 `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` 创建的子进程在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）死亡；子进程共享宿主控制台，基于管道的 stdio 重定向不受影响。当启动它的宿主没有控制台、子进程只能自行分配一个控制台时，该窗口通过 `STARTF_USESHOWWINDOW` 与 `SW_HIDE` 保持隐藏：这是启动提示，而非控制台隔离。
