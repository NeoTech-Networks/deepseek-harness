...
- **The encoding preamble precedes the command** — PowerShell requires `param(...)`, `#requires`, and `using` statements at the very top of a script, and the UTF-8 output preamble owns line 1. A `param(...)`-led command is wrapped in a script block automatically, so it runs unchanged; `#requires` is inert under `-Command`, and a `using` statement must be run from a file instead.
