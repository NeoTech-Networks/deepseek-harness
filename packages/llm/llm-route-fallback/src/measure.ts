 ...
 * parameter JSON schemas, which is the quantity the wire eventually carries:
 * the DeepSeek adapter sends `tool.parameters` as each tool's `input_schema`
 * on the Messages wire with no transformation in between. Tools marked
 * `deferLoading` are not sent in full, so they are not counted. Nothing here truncates,
...
export function toolParameterBytes(tools: readonly ToolSchema[]): number {
  ...
  for (const tool of tools) {
    if (tool.deferLoading === true) continue
