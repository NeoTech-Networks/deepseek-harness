...
describe('toolParameterBytes', () => {
  ...
  it('skips deferred tools, whose schemas are not sent in full', () => {
    const parameters = { command: { type: 'string' } }
    const deferred: ToolSchema = { ...tool('d', { big: { type: 'string', description: 'x'.repeat(500) } }), deferLoading: true }
    expect(toolParameterBytes([tool('a', parameters), deferred]))
      .toBe(Buffer.byteLength(JSON.stringify(parameters), 'utf8'))
  })
})
