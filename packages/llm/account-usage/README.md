...
Shows how much of a subscription account's rolling and weekly rate limits the operator has consumed, as one short reading under the composer (`5h 3% · Week 16%`) with a click-open panel of reset instants, scoped weekly windows, and the month's extra-usage spend. Choose it when a provider authorizes by subscription grant: without it, the first sign of exhaustion is a refusal mid-turn. The Host half reads the stored grant and never refreshes it, so an expired grant shows as stale until the next model call. A deployment with no subscription grant shows nothing.
