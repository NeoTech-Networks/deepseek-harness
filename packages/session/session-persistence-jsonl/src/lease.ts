...
async function flockAsync(fd: number, flags: 'exnb' | 'un'): Promise<void> {
  // fs-ext is a node-gyp native module. Load it lazily so importing this module
  // does not require the compiled binding on Windows, where acquire() takes the
  // win32 koffi-semaphore branch above and flock(2) is never called.
