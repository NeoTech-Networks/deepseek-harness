...
- **不让任何控制台窗口出现在屏幕上** — 每条路径都设置 `STARTF_USESHOWWINDOW` 与 `wShowWindow = SW_HIDE`，因此当启动它的宿主自身没有控制台、子进程只能自行分配一个控制台时，该窗口保持隐藏。ordinary 路径另外传入 `CREATE_NO_WINDOW`；restricted-token 路径不能这样做，因为这样的子进程会在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED` 死亡，所以覆盖它们的是 show-window 请求而不是创建标志。
