...
- **No console window reaches the screen** — every path sets `STARTF_USESHOWWINDOW` with `wShowWindow = SW_HIDE`, so a console the child allocates (its launching host has none of its own) stays hidden. The ordinary path additionally passes `CREATE_NO_WINDOW`; the restricted-token paths must not, because such a child dies during DLL initialization with `STATUS_DLL_INIT_FAILED`, which is why the show-window request rather than a creation flag is what covers them.
