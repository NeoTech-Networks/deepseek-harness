 ...
 *
 * The root list and the explorer preference are this plugin's own volatile
 * Config fields on the profile entry `pinned-files`, so they persist in the
 * active profile's cordis patch through the settings service, survive restarts,
 * are identical in every Session and every Workspace, and can be edited by hand
 * without this service running. A legacy `pinned-files:` section of
 * `$DSH_HOME/settings.yaml` imports into the same entry on first boot.
 */
...
// Type-only: the optional `settings` service that persists edits into this
// entry, and the Loader's `fiber.entry` merge that names the entry.
...
/** The operator-owned fields, read as plain values. */
...
/**
 * The profile entry id this service is mounted under by the shipped bundle,
 * and the section name a legacy `settings.yaml` imports into.
 */
export const name = 'pinned-files'
...
export class PinnedFiles extends TypertRemoteService {
  ...
  static Config = z.object({
    ...
    roots: z.array(z.string()).default([]).volatile(),
    autoOpen: z.boolean().default(false).volatile(),
  })

  /** Profile entry that owns the live fields; absent when mounted outside a Loader entry. */
  private readonly entryId: string | undefined

  /**
   * @param ctx - Host context carrying the filesystem and, optionally, the settings service.
   * @param config - deployment caps plus the operator's live fields.
   */
  ...
  /** The live fields as they stand, with schema defaults already applied. */
  private section(): PinnedSection {
    return { roots: [...this.config.roots.get()], autoOpen: this.config.autoOpen.get() }
  }
  ...
  /**
   * One write into this plugin's profile entry through the settings service,
   * with the operator-facing refusal an absent or read-only profile earns.
   */
  private async persist(patch: Partial<PinnedSection>, path: string): Promise<void> {
    ...
    if (settings === undefined || this.entryId === undefined) {
      throw new RemoteError(
        ...
        'no settings service or profile entry is available to store the change',
        { path },
      )
    }
    try {
      await settings.update(this.entryId, patch)
    } catch (error: unknown) {
      throw new RemoteError(
        'pinned-files/not-writable',
