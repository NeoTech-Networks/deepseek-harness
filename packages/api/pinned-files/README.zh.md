...
本服务保存操作者在 Host 机器上固定的目录清单，并在这些目录中列目录、读文件。清单是本条目自己的实时 Config（`roots`、`autoOpen`），经由设置服务保存到当前 profile：重启后依然存在，在每个 Session 和每个 Workspace 中都相同，也可以手工编辑。这里没有任何东西按 Session 作用域，这正是重点；按 Session 作用域的文件服务已经存在，即 [`dsh-api-workspace-files`](../workspace-files/README.zh.md)。
...
以条目 id `pinned-files` 挂载一行。组合中挂载了设置服务时，编辑会经由它持久化；由于两个字段都是 volatile，`$DSH_HOME/settings.yaml` 中旧的 `pinned-files:` 一节会在首次启动时导入到本条目。
