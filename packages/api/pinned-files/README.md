...
Mount one row with the entry id `pinned-files`. Edits persist through the settings service when the composition mounts it; a legacy `pinned-files:` section of `$DSH_HOME/settings.yaml` imports into this entry on first boot because both fields are volatile.
...
**Runtime invariant:** No companion is published. The root list has exactly one home, this entry's live Config, and every write goes through `settings.update` on that entry, so there is no second observation of it to compare against.
