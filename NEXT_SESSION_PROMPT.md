2026-09-21 POINTER (supersedes everything below, which is kept as the 2026-09-11 record): the plan-mode lead-in fix is INSTALLED AND PROVEN IN THE RUNNING BUILD. `finish-install.ps1` was run from `C:\Projects\worktrees\dsh-plan-preamble` on 2026-09-21 and exited 0 (installer 194,900,330 bytes, SHA256 `30108958927725C2E91ABC0A8A9DCAF6265368C7806BCAB0D2032995AB9F37C1`, re-hashed before it ran); the seed verified, the profile was re-extracted, and the app relaunched as 0.1.5-rc.2. Proof: `node C:\Projects\temp\plan-fault-proof.mjs` reads ACCEPTED for the note-then-title and sentence-then-title cases and REFUSED for the other four; `py C:\Claude\bin\dsh_local_features_check.py` reads all 28 features present, exit 0; `py C:\Claude\bin\dsh_config_vault.py verify` reads 20 files all same, exit 0. That install also carried item 24's four composer silence guards, so ITEM 24 IS CLOSED. Evidence: `C:\Projects\logs\2026-09-21\dsh-plan-preamble-install\`; the pre-install control stays at `C:\Projects\logs\2026-09-20\dsh-plan-preamble\`. DO NOT re-run the installer: it force-closes the app, and the build it would write is the same one already running.
