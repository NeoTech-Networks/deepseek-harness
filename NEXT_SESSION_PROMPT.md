Continue deepseek-harness. 0.1.5-alpha.2 IS BUILT AND PACKAGED BUT NOT INSTALLED. It was REPACKAGED on 2026-09-09 to carry the Alt+S / Alt+P shortcut fix (commit aca41e5f7b) on top of everything below; the artifact name and size are unchanged. The shortcuts never worked in the desktop app and no update broke them: Electron on Windows never delivers the KEYDOWN of an Alt+letter chord to the renderer, only a keyup carrying altKey, so the fix binds keyup. Do not re-diagnose that; the full evidence is in VERIFICATION_RESULTS.md and OPEN_ISSUES item 2. After the install, run `py C:\Claude\bin\dsh_local_features_check.py` (must exit 0) and `py C:\Claude\bin\dsh_config_vault.py verify` (must exit 0) alongside the six rows below, then have Steve press Alt+S once. Branch update/v0.1.5-alpha.2 in worktree C:/Projects/worktrees/dsh-update-v0.1.5-alpha.2 (pushed to the fork) carries the local stack rebased onto dsh-v0.1.5-alpha.2 plus four fixes that had never shipped: session-status icons e5146450e3, first-run provisioning f82bb8df30 and f291778191, console-window suppression 5e10c7c560. Gates all green: typecheck 0, build 0, plan-mode 94/94, fs-local 156 passed 0 failed, 594 targeted tests, packaging 0. Installer deepseek-harness-0.1.5-alpha.2-win-x64.exe, 194,655,398 bytes, app id com.deepseek.harness. SessionPanelPhase was read back out of the packaged .tgz, so the fix is in the artifact. DO NOT rebuild, re-rebase or re-package.
